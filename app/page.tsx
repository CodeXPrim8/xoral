...
export default function Page() {
  ...
  useEffect(() => {
    async function load() {
      const followed = await Promise.all(
        catalog.creators.map(async (creator) => ((await isCreatorFollowed(creator.id)) ? creator.id : null))
      );
  ...
  return (
    <div className="bg-background text-foreground">
      ...
      <main className="xoral-page space-y-8 py-6 pb-24 md:pb-8">
        <ContentCarousel title="Your Next Watch" items={catalog.nextWatch} viewAllHref="/library" />
        <ContentCarousel
          title="Trending Now"
        ...
        <section className="space-y-6">
          ...
          <ContentCarousel items={catalog.aiMovies} title="" viewAllHref="/ai-stars" />
        </section>
        <MeetTheCast />
        <section className="space-y-6">
          <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-4 gap-4 xl:gap-6">
            {catalog.creators.map((creator) => {
              ...
              return (
                <div key={creator.id} className="glass-card p-4 rounded-xl border text-center hover:bg-card/60 smooth-transition">
                  <SafeImage
                    src={creator.image}
                    alt={creator.name}
        ...
        <FeaturedGrid title="More to Explore" items={catalog.trending.slice(0, 8)} />
      </main>
      <MobileNav />
    </div>
  );
